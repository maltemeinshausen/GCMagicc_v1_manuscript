...
def plot_delta_mean_pdf(
    ...
    *,
    ...
    use_graticule_if_no_coastlines: bool = False,  # <- changed default to match your original behavior
    ...
    - Else: healpy.mollview. (healpy does not provide true coastlines.)
    """
    hp_map = np.asarray(hp_map, dtype=np.float32)
    ...
    if add_coastlines:
        try:
            ...
            lons = (0.5 + np.arange(nlon)) / nlat * 180.0  # matches your earlier lon def
            ...
            # Plot as raster inside PDF (keeps coastlines vector + file size reasonable)
            pm = ax.pcolormesh(
                lon2d,
                lat2d,
                data_grid,
    ...
    # ---- healpy fallback (restored "clean" look: no style-colored background, no graticule by default) ----
    fig = plt.figure()
    fig.patch.set_facecolor("white")

    # Try to be robust across healpy versions (bgcolor/fig args may differ)
    try:
        hp.mollview(
            hp_map,
            fig=fig.number,
        ...
        # Older healpy: retry without bgcolor / fig kwargs if needed
        try:
            hp.mollview(
                hp_map,
                fig=fig.number,
...
def sample_T1new(
    x,
    device="cuda:0",  # default; 'cuda:o' -> 'cuda:0'
    dirname="../model_NthreeversT1/modelsA/",
    DATE="7Augext",
    dependence=False,
    nside=64,
    rectangular=True,
    ...
    *,
    nsim: int = 1,
    reduce: str | None = None,  # None | 'max' | 'maxtas' | 'mean' | 'min'  (over time)
):
    dev = torch.device(device) if torch.cuda.is_available() else torch.device("cpu")
    dirm = os.path.join(dirname, "modelsNthree_")
    ...
    def _reduce_over_time(y: torch.Tensor, how: str):
        ...
        if how == "maxtas":
            ...
            if y.size(1) <= 1:
                raise ValueError("need C>=2 for 'maxtas' (uses channel index 1 as reference)")
    ...
    cfg = {
        1: dict(kind="L4de", maxlag=48, variab=10, add_latent_dim=40),
        2: dict(kind="L4de", maxlag=48, variab=10, add_latent_dim=40),
        4: dict(kind="L4de", maxlag=6, variab=0, add_latent_dim=10),
        8: dict(kind="L4de", maxlag=2, variab=0, add_latent_dim=5),
        16: dict(kind="L4adde"),
        32: dict(kind="L4adde"),
        64: dict(kind="L4adde"),
        128: dict(kind="L4addemini"),
        256: dict(kind="L4addemini"),
    }
    ...
    def _load_level(nside_hi: int):
        nside_lo = nside_hi // 2
        if nside_hi in (1, 2, 4, 8, 16, 32, 64):
            if nside_hi <= 8:
                modelb = _prep_model(
                    LB2(nside_lo, nside_hi, n_features=n_features), dirm + DATE + f"_b{nside_hi}"
                )
                modelm = _prep_model(
            ...
            if k == "L4de":
                mcfg = cfg[nside_hi]
                lags = [i for i in range(1, mcfg["maxlag"])]
                modelcore = _prep_model(
                    L4de(
                        ...
                        variab=mcfg["variab"],
                        lags=lags,
                        add_latent_dim=mcfg["add_latent_dim"],
                    ),
                    dirm
                    + DATE
                    + f'_l{nside_hi}_{mcfg["maxlag"]}_{mcfg["variab"]}_{mcfg["add_latent_dim"]}',
                )
            elif k == "L4adde":
                modelcore = _prep_model(
                ...
                raise RuntimeError("Unexpected kind for this level.")
        else:
            modelbias = _prep_model(
                LB2mini(nside_lo, nside_hi, n_features=n_features), dirm + DATE + f"_b{nside_hi}"
            )
            modelcore = _prep_model(
    ...
    def _one_sim(sim_seed_offset: int | None):
        ...
        with torch.inference_mode():
            ...
            if reduce is not None:
                yh_obs = _reduce_over_time(yh_obs, reduce)  # (C,P)
    ...
    if reduce is None:
        out = sims[0] if nsim == 1 else torch.stack(sims, dim=0)  # (T,C,P) or (nsim,T,C,P)
    else:
        out = sims[0] if nsim == 1 else torch.stack(sims, dim=0)  # (C,P) or (nsim,C,P)
...
def main():
    ...
    STORE_COUNTERFACTUALS_DEFAULT = False  # store counterfactual/actual means only if True
    ADD_COASTLINES_DEFAULT = False

    # ---- your run config (kept close to your original) ----
    DATE = "7Augext"

    # ERA5 normalized file you used
    H5_PATH = (
        "external/nicolai-data/cmip6/normalized_"
        + DATE
    ...
    # model selection (your current loop uses only T1new)
    MODELS = ["T1new"]
    ...
    # ---- output variables ordering (unchanged) ----
    ...
    # ---- load transformation scalars (robust: try local then model dir) ----
    meta_local = Path(f"meta_{DATE}.pkl")
    meta_modeldir = Path("../model_NthreeversT1/modelsA/") / f"meta_{DATE}.pkl"
    meta_path = meta_local if meta_local.exists() else meta_modeldir
    ...
    # ---- pre/post for y_obs (kept from your code) ----
    ...
    # You computed this but then did not use ymax in delta; keeping minimal
    y = (yo - post_constant[None, :, None]) * inv_pre[None, :, None]
    _ = y.mean(dim=0)

    # indices for tasmax / tasmin
    ...
    for cof in COFS:
        for mc in MC_RANGE:
            for model in MODELS:
                ...
                # ---- sample (only T1new used here) ----
                if model != "T1new":
                    raise ValueError("This modified file currently implements only model='T1new' as in your loop.")
                ...
                # yh, ycf are (nsim, C, P) because reduce="mean"
                if yh.ndim != 3:
                ...
                # ---- default requested: subtract tasmin -> tasmax becomes tasmax - tasmin ----
                if subtract_tasmin:
                    yh = yh - yh[:, tasmin_idx : tasmin_idx + 1, :]
                    ycf = ycf - ycf[:, tasmin_idx : tasmin_idx + 1, :]

                # ---- extract tasmax only ----
                yh_v = yh[:, tasmax_idx, :]  # (nsim, P)
                ycf_v = ycf[:, tasmax_idx, :]  # (nsim, P)

                # ---- mean effect only (requested) ----
                delta_mean = (yh_v.mean(dim=0) - ycf_v.mean(dim=0)).numpy().astype(np.float32)  # (P,)

                # ---- also compute means if storing counterfactuals ----
