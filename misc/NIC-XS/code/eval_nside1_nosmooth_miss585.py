"""
Evaluate the nside=1 NOSMOOTH model (no tas_smoothed/rtmt_smoothed predictors).
Same plots as eval_nside1.py but for the nosmooth variant.
"""
...
normalized_dir = SCRATCH_BASE + '/userdata/nicolai/cmip6/normalized_' + DATE + '/'
model_base = SCRATCH_BASE + '/userdata/nicolai/cmip6/modelsNfour_' + DATE + SUFFIX + '_nosmooth'
plot_dir = os.path.join(os.path.dirname(__file__), 'plots_eval_nside1_nosmooth_miss585' + _args.tag)
os.makedirs(plot_dir, exist_ok=True)

var_names = ["psl", "tas", "pr", "sfcWind", "ts", "tasmin", "tasmax", "rsds", "hurs", "huss"]

with open(os.path.join(os.path.dirname(__file__), 'meta_' + DATE + '.pkl'), 'rb') as f:
    meta = pickle.load(f)
...
def downsample_to_nside2(y64):
    ...
    for _ in range(6):  # 64->32->16->8->4->2->1
        y = downs(y)
    return y
...
@torch.no_grad()
def predict_file(bias_model, effect_model, fpath, n_draws=10):
    ...
    return {
        'obs': y2_obs,               # (T, F, 48)
        'bias': y_bias,              # (T, F, 48)
        'draws': torch.stack(draws), # (n_draws, T, F, 48)
        'draws_ar': torch.stack(draws_ar),
...
def main():
    ...
    for scenarios, label in [(test_scenarios, 'TEST'), (train_scenarios, 'TRAIN')]:
        for scenario, sty in scenarios.items():
            for model_name in eval_models:
                files = find_files(model_name, scenario, max_files=1)
                if not files:
                    continue
                fpath = files[0]
                key = (scenario, model_name)
                print(f"[{label}] {model_name} / {scenario} ({os.path.basename(fpath)})")
                try:
                    result = predict_file(bias_model, effect_model, fpath, n_draws=10)
    ...
    import pandas as pd
    csv_rows_full = []
    for (scen, mname), res in sorted(all_results.items()):
        is_test = res['is_test']
        T = res['T']
        for vi, vn in enumerate(var_names):
            obs_monthly = inv_transform(global_mean(res['obs'])[:, vi].numpy(), vi)
            pred_monthly = inv_transform(
                global_mean(res['draws'].mean(dim=0))[:, vi].numpy(), vi)
            pred_lo = inv_transform(
                np.percentile(global_mean(res['draws'])[:, :, vi].numpy(), 10, axis=0), vi)
            pred_hi = inv_transform(
                np.percentile(global_mean(res['draws'])[:, :, vi].numpy(), 90, axis=0), vi)
            for t in range(T):
                csv_rows_full.append({
                    ...
                    'pred_p10': float(pred_lo[t]),
                    'pred_p90': float(pred_hi[t]),
                })
    ...
    for var_idx, var_name in enumerate(var_names):
        ...
        fig.suptitle(f'[NOSMOOTH] Global mean {var_name} at nside=1: observed (solid) vs predicted (dotted, 10-90% band)',
                     fontsize=13, y=1.01)
        fig.tight_layout()
    ...
    for vi, vshort, vlong, vunit in plot_vars:
        ...
        for model_name in eval_models:
            ...
            for (scen, mn), res in sorted(model_data.items()):
    ...
    scenario_rmse = {}
    for (scen, mname), res in all_results.items():
    ...
    for model_name in eval_models:
        ...
        for var_idx, var_name in enumerate(var_names):
            ...
            for (scen, mname), res in sorted(model_results.items()):
    ...
    if n_models_found > 0:
        ...
        for model_name in eval_models:
            ...
            scen_rmse = {}
            for (scen, mname), res in model_results.items():
                rmses = []
                for vi, vn in enumerate(var_names):
                    obs = inv_transform(global_mean(res['obs'])[:, vi].numpy(), vi)
                    pred = inv_transform(global_mean(res['draws'].mean(dim=0))[:, vi].numpy(), vi)
