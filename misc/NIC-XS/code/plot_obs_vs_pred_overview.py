"""
Combined multi-panel obs-vs-pred overview.
...
for var in variables:
    ...
    for idx, model_name in enumerate(models):
        ...
        for scen, sdf in mdf.groupby('scenario'):
            ...
            split[key].append((scen, obs, pred))
            if is_test:
        ...
        for ax, label, data in [(ax_tr, 'Train', split['train']),
            ...
            for scen, obs_v, pred_v in data:
                c = scenario_colors.get(scen, 'black')
                ax.scatter(obs_v, pred_v, c=c, s=1.5, alpha=0.25,
    ...
    fig.suptitle(f'{prefix}Obs vs Pred {var.upper()} (monthly, all models)',
                 fontsize=12, y=1.0)
    fig.legend(handles=handles, loc='upper center', ncol=min(len(handles), 10),
               fontsize=6, frameon=False, bbox_to_anchor=(0.5, 0.97))

    out_name = os.path.join(out_dir, f'obs_vs_pred_{var}_overview{args.tag}.png')
