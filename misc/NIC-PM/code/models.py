...
class L4de(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.mlp_x_reduce = make_mlp(
            (x_features-3) + noise_x, x_dim, hidden_x, depth_mlp_x)
    ...
    def _forward_single(self, x, y_in, *, prev_list=None):
        ...
        # ===== encode x (drop idx cols) =============================
                
        idx = torch.cat([
            torch.arange(1, 4, device=x.device),          # 1, 2, 3
            torch.arange(6, x.size(1), device=x.device)   # 6, 7, 8, ..., n_features-1
        ])
        noise_vec = torch.randn(B, self.noise_x, device=dev)
        x_red = self.mlp_x_reduce(torch.cat([x[:, idx], noise_vec], 1))   # (B,x_dim)
        ...
        # (B,P,add_latent)
        if prev_list and (not any(p is None for p in prev_list)) and (torch.rand(1, device=dev) < 0.995):
...
class M1(nn.Module):
    """
    Residual per-pixel correction from (x_without_model, theta, phi, y_at_pixel).

    Args:
        nside (int): HEALPix nside (defines number of pixels P).
        n_features (int): number of channels in y (C).
        x_features (int): total features in x (including the model-index col at 0).
        mlp_hidden (int): hidden width for the MLP.
        mlp_depth (int): number of hidden layers for the MLP (purely deterministic).

    Input shapes:
        y : (B, C, P)
        x : (B, x_features)  -- column 0 (model index) is ignored

    Output:
        y_out : (B, C, P)  -- same shape as y (residual added)
    """
    def __init__(
        self,
        nside: int,
        *,
        ...
        if self.x_features < 2:
            raise ValueError("x_features must be at least 2 (so we can drop column 0).")

        # Precompute per-pixel (theta, phi) and store as a buffer
        # theta in [0, π], phi in [0, 2π)
        pix = np.arange(self.npix, dtype=np.int64)
        theta, phi = hp.pix2ang(self.nside, pix, nest=False)   # use RING by default
        ang = np.stack([theta, phi], axis=1).astype(np.float32)  # (P, 2)
        self.register_buffer("angles", torch.from_numpy(ang))     # (P,2)

        # MLP input: (x_features - 1)  +  2 (theta,phi)  +  n_features (y at pixel)
        mlp_in_dim = (self.x_features - 1) + 2 + self.n_features
        self.mlp = make_mlp(mlp_in_dim, self.n_features, mlp_hidden, mlp_depth)

    @torch.no_grad()
    def _check_shapes(self, y: torch.Tensor, x: torch.Tensor):
        if y.dim() != 3:
            raise ValueError(f"y must be 3-D (B,C,P); got shape {tuple(y.shape)}")
        if x.dim() != 2:
            raise ValueError(f"x must be 2-D (B,X); got shape {tuple(x.shape)}")
        B, C, P = y.shape
        if P != self.npix:
            raise ValueError(f"y has P={P}, but nside={self.nside} implies P={self.npix}.")
        if C != self.n_features:
            raise ValueError(f"y has C={C}, expected n_features={self.n_features}.")
        if x.size(0) != B:
            raise ValueError("Batch size mismatch between y and x.")

    def forward(self, y_in: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        """
        Deterministic residual update:
            y_out(b, :, p) = y(b, :, p) + MLP([x_wo_model(b, :), theta[p], phi[p], y(b, :, p)])
        """
        self._check_shapes(y_in, x)
        B, C, P = y_in.shape
        dev, dtype = y_in.device, y_in.dtype

        # Drop the first column of x (model index)
        x_wo = x[:, 1:]                               # (B, X-1)
        # Broadcast x across pixels
        x_exp = x_wo.unsqueeze(1).expand(B, P, -1)    # (B, P, X-1)

        # Per-pixel angles (theta, phi), broadcast across batch
        ang = self.angles.to(device=dev, dtype=dtype).unsqueeze(0).expand(B, -1, -1)  # (B, P, 2)

        # y at pixel p
        y_pix = y_in.permute(0, 2, 1).contiguous()       # (B, P, C)

        # Build MLP input per pixel
        mlp_in = torch.cat([x_exp, ang, y_pix], dim=2)     # (B, P, (X-1)+2+C)

        # Run MLP in a flat batch over pixels
        mlp_in_flat = mlp_in.view(B * P, -1)               # (B*P, D_in)
        delta_flat  = self.mlp(mlp_in_flat)                # (B*P, C)
        delta       = delta_flat.view(B, P, C).permute(0, 2, 1).contiguous()  # (B, C, P)
